...
int install_snell() {
    KernelUpdate();
    system("setenforce 0");
    system("apt install -y curl pwgen unzip dnsutils");
...
int KernelUpdate() {
    if ((fopen("KernelUpdate.sh", "r")) == NULL) {
        printf("正在升级新内核. . .\n");
        system("wget https://raw.githubusercontent.com/HXHGTS/TCPOptimization/master/KernelUpdate_debian10.sh -O KernelUpdate.sh");
        system("chmod +x KernelUpdate.sh");
        printf("正在升级，将自动触发重启以应用配置. . .\n");
        system("bash KernelUpdate.sh");
    }
    else {
        system("curl -sSL https://raw.githubusercontent.com/HXHGTS/TCPOptimization/master/TCPO_debian10.sh | sh");
    }
    return 0;
}
